...
class CTOD():
    ...
    def pre_process_full_context(self):
        # 4. Pre-process context frames
        self._context = [self.pre_process_context(
    ...
    def pre_process_context(self, obs: np.array, bb: np.array = None):
        ...
        obs = ToTensor()(copy.deepcopy(obs))
        # ---- Resized crop ----#
        img_res = resized_crop(obs, top=top, left=left, height=box_h,
                               width=box_w, size=(100, 180))
        cv2.imwrite(os.path.join(os.path.dirname(
            os.path.abspath(__file__)), "resize_cropped.png"), np.array(np.moveaxis(
                copy.deepcopy(img_res).cpu().numpy()*255, 0, -1), dtype=np.uint8))
        adj_bb = None
        # if bb is not None:
    ...
    def pre_process_obs(self, obs: np.array, bb: np.array = None):
        ...
        img_res = ToTensor()(cv2.resize(cropped_img, (180, 100)))
        cv2.imwrite(os.path.join(os.path.dirname(
    ...
    def get_bb(self, obs: np.array, robot_state: np.array):
        """_summary_
        ...
        inputs['demo'] = context
        inputs['images'] = obs
        inputs['gt_bb'] = torch.from_numpy(
            np.array([[-1, -1, -1, -1]]))[None][None].cuda(0)
        inputs['gt_classes'] = torch.from_numpy(
        ...
        with torch.no_grad():
            ...
            if max_score_target[0] != -1:
                ...
                scale_factor = self._model.get_scale_factors()
